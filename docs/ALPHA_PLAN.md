...
| P1 | Standalone app works without development tools; install/upgrade/uninstall preserve chosen state | Exact `b603442` ZIP [build and stripped-guest checks](../packages/cli/packaging/README.md) passed offline recall, targeted error/recovery paths, and the [logged-in GUI journey](validation/desktop-gui.md); prior `d1c3e3b` [guest run](validation/packaged-app-2026-09-23.md) passed daemon, consent, graceful reboot, uninstall/key preservation, and locked-Keychain fail-fast. Legacy 0.1 wrapped-key approval remains unproven | Partial; upgrade pending |
