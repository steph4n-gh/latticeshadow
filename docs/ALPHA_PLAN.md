...
| --- | --- | --- | --- |
| M1 | Stable IDs and correct occurrence/ingestion times across sources, migration and rebuild | Final runtime `b603442`: DB 228 and CLI 241 tests pass, including timeline/rebuild contracts | Passed locally |
| M2 | Filters correct before ranking/limits; zero excluded records returned | CLI/UI/MCP tests pass; the exact `b603442` [packaged GUI journey](validation/desktop-gui.md) exercised source and project filters plus excluded-record empty state; prior guest checks covered Unassigned and time filters | Passed locally and on final packaged GUI |
| M3 | Cross-process writes/deletes/rebuilds stay consistent; no resurrection after fault/restart | Runtime `b603442` in source HEAD `faa91b5`: `validate_lifecycle.py lifecycle --report benchmark_results/alpha-lifecycle-b603442.json` 12/12 on Mac16,8, macOS 27.0; raw report ignored; driver SHA-256 `a102971832a53b397d53e2ab7355a72840235f69f660f82ea8924dc53cf33c15` | Passed locally on candidate runtime |
| M4 | Retention/exclusions/consent/pause behave as documented; experiments off by default | Final CLI suite covers consent epochs, read-time clipboard revocation, retention, and pause/resume; exact `b603442` [stripped-guest run](validation/legacy-upgrade-and-lifecycle-2026-09-23.md) proved explicit source choices, later-copy capture, and pause/source choices across a graceful reboot. The prior [candidate run](validation/packaged-app-2026-09-23.md) also checked pre-consent exclusion. Automatic startup after a desktop login remains untested. | Scoped pass; login startup pending |
| M5 | Portable recovery preserves canonical fields in fresh guest without source key; failures leave old vault usable | Exact `b603442` ZIP [fresh-guest recovery](validation/mini-b603-packaged-2026-09-23.md): retained canonical event JSON SHA matched, tombstone persisted, wrong passphrase left no destination and good restore usable | Passed on final candidate artifact |
...
| R2 | Warm user-visible search p95 below 500 ms over 10,000 events on the Mini; cold cost reported separately | Exact `b603442` ZIP [packaged MCP run](validation/mini-b603-packaged-2026-09-23.md): 100 full calls, p95 300.57 ms, cold first 5.36 s; the earlier [source-only result](validation/mini-source-performance-2026-09-23.md) measures a narrower call | Passed on final candidate artifact |
| U1 | Actual keyboard/menu → filter/preview/copy/open/assign/forget flow works; stale searches cannot overwrite state | Exact `b603442` [installed guest journey](validation/desktop-gui.md) passed menu/filter/preview/copy/file Open/assign/confirmed Forget and observed latest rapid-query result. Cross-app shortcut was intercepted by Finder through Screen Sharing; a deterministic visual out-of-order completion race remains unproven | Partial; keyboard/race pending |
...
| P1 | Standalone app works without development tools; install/upgrade/uninstall preserve chosen state | Exact `b603442` ZIP [build and stripped-guest checks](../packages/cli/packaging/README.md) passed offline recall, targeted error/recovery paths, the [logged-in GUI journey](validation/desktop-gui.md), and [explicit daemon/reboot/removal checks](validation/legacy-upgrade-and-lifecycle-2026-09-23.md). Its populated 0.1 upgrade failed to reopen a wrapped key after ad-hoc signature replacement, while preserving key/vault; even reinstalled 0.1 failed. Signed stable-identity migration remains unverified. | Partial; signed upgrade pending |
...
| E1 | Integrated cross-surface journey and 2-hour candidate soak pass; resource behavior reported | CLI/MCP/delete/restart journey and exact `b603442` [packaged desktop journey](validation/desktop-gui.md) passed; the final-runtime 7,200-second source soak remains in progress. | Pending soak |
| E2 | Standard suites/docs, model tests, native/build checks and final independent review pass for integrated change | Final runtime `b603442`: DB 228, CLI 241, real model 2, docs and CI selector 7 pass locally; native release build passed at `dd079b3`; exact final app verifier passed 307 linked binaries; independent review findings addressed. PR CI pending. | Local pass; PR CI pending |
