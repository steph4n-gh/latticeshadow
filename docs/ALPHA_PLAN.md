...
| --- | --- | --- | --- |
| M1 | Stable IDs and correct occurrence/ingestion times across sources, migration and rebuild | `20669ff` source: DB 228 and CLI 242 tests passed, including timeline/rebuild contracts; the later keyboard-only source passed CLI 244. | Passed locally |
| M2 | Filters correct before ranking/limits; zero excluded records returned | CLI/UI/MCP tests pass; `20669ff` [packaged GUI journey](validation/desktop-gui.md) exercised project filter. Preceding `b603442` packaged journey exercised source/project filters and excluded-record empty state; earlier guest checks covered Unassigned and time filters. | Passed locally; packaged project filter passed on earlier ZIP |
| M3 | Cross-process writes/deletes/rebuilds stay consistent; no resurrection after fault/restart | `20669ff`: `validate_lifecycle.py lifecycle --report benchmark_results/alpha-lifecycle-20669ff.json` 12/12 on Mac16,8, macOS 27.0; raw report ignored; driver SHA-256 `a102971832a53b397d53e2ab7355a72840235f69f660f82ea8924dc53cf33c15`. Later source change affects only GUI keyboard handling. | Passed locally on unchanged core path |
| M4 | Retention/exclusions/consent/pause behave as documented; experiments off by default | CLI suite covers consent epochs, read-time clipboard revocation, retention, and pause/resume; `20669ff` [stripped-guest run](validation/packaged-app-20669ff-2026-09-23.md) proved pre-enable exclusion, later-copy capture, pause, and source choices across a normal reboot. A paused copy added no event. | Passed in source and preceding stripped guest |
| M5 | Portable recovery preserves canonical fields in fresh guest without source key; failures leave old vault usable | `20669ff` ZIP [exported and restored its own synthetic archive across fresh guests](validation/packaged-app-20669ff-2026-09-23.md), preserving two retained canonical events and one tombstone. It also restored a `b603442` archive. Wrong passphrases created no destination and left the successful restores usable. The keyboard-fixed ZIP did not repeat recovery. | Passed on predecessor ZIP; exact latest ZIP pending |
...
| R2 | Warm user-visible search p95 below 500 ms over 10,000 events on the Mini; cold cost reported separately | `20669ff` ZIP [packaged MCP run](validation/packaged-app-20669ff-2026-09-23.md): 100 full calls, p95 289.19 ms, cold first 5.4 s. The keyboard-fixed ZIP did not repeat the 10,000-event run. | Passed on predecessor ZIP; exact latest ZIP pending |
| U1 | Actual keyboard/menu → filter/preview/copy/open/assign/forget flow works; stale searches cannot overwrite state | `20669ff` [installed guest journey](validation/desktop-gui.md) passed menu/Recall/search/project filter/preview/Copy/Open/Assign/confirmed Forget/consent controls. The [keyboard-fixed `14aa7a3` ZIP](validation/packaged-app-14aa7a3-2026-09-23.md) passed focused-row Return and keypad Enter copying. A rapid-query sequence settled on the latest query. Cross-app shortcut and deterministic visual out-of-order completion race remain unproven. | Partial; cross-app shortcut and race evidence pending |
...
| P1 | Standalone app works without development tools; install/upgrade/uninstall preserve chosen state | `20669ff` ZIP [stripped-guest checks](validation/packaged-app-20669ff-2026-09-23.md) passed offline recall, safe shred wording, query, export/restore, daemon/consent/reboot/remove, plus a [logged-in GUI journey](validation/desktop-gui.md). The [keyboard-fixed `14aa7a3` ZIP](validation/packaged-app-14aa7a3-2026-09-23.md) passed verification, stripped-guest offline recall, and installed keyboard actions. Populated 0.1 upgrade failed after ad-hoc signature replacement while 0.2 preserved key/vault; even reinstalled 0.1 failed. Signed stable-identity migration remains unverified. | Partial; signed upgrade and exact latest ZIP lifecycle pending |
...
| E1 | Integrated cross-surface journey and 2-hour candidate soak pass; resource behavior reported | `20669ff` source CLI/MCP/delete/restart lifecycle passed 12/12 and its [packaged desktop journey](validation/desktop-gui.md) passed Open/Assign/Forget. The `b603442` predecessor [local core-path source soak](validation/local-b603-soak-2026-09-23.md) completed 7,200.85 seconds with final live/deleted-ID and resource checks passed. Current-revision elapsed soak remains separate. | Predecessor core soak passed; current-revision soak pending |
| E2 | Standard suites/docs, model tests, native/build checks and final independent review pass for integrated change | Through `7f131b7`: CLI 244 and docs pass. At `20669ff`, the unchanged DB core passed 228, real model 2, lifecycle 12, and CI selector 7 locally. Native release build passed at `dd079b3`; the `14aa7a3` app verifier passed 307 linked binaries. Independent review findings were addressed. PR CI pending. | Local pass; PR CI pending |
