...
| U1 | Actual keyboard/menu → filter/preview/copy/open/assign/forget flow works; stale searches cannot overwrite state | `0af8643` [logged-in guest journey](validation/desktop-gui.md) passed menu, filters, preview, copy, truthful Open, assign, forget; global shortcut and rapid visual stale-race not proven; final styled artifact pending | Partial guest pass |
