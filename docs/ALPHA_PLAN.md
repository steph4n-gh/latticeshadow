...
| U1 | Actual keyboard/menu → filter/preview/copy/open/assign/forget flow works; stale searches cannot overwrite state | Tested `20669ff` [installed guest journey](validation/desktop-gui.md) passed menu/Recall/search/project filter/preview/Copy/Open/Assign/confirmed Forget/consent controls. A rapid-query sequence settled on the latest query. Escape closed Recall. Return with a result row focused did not copy in that ZIP; `bc5c2c3` fixes it in source with 23 focused GUI tests passed. Cross-app shortcut, packaged Return retest, and deterministic visual out-of-order completion race remain unproven. | Partial; packaged keyboard and race evidence pending |
...
| P1 | Standalone app works without development tools; install/upgrade/uninstall preserve chosen state | Tested `20669ff` ZIP [build and stripped-guest checks](validation/packaged-app-20669ff-2026-09-23.md) passed offline recall, safe shred wording, query, export/restore, daemon/consent/reboot/remove, plus a [logged-in GUI journey](validation/desktop-gui.md). Populated 0.1 upgrade failed after ad-hoc signature replacement while 0.2 preserved key/vault; even reinstalled 0.1 failed. Signed stable-identity migration remains unverified. | Partial; signed upgrade pending |
