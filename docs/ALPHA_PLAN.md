...
| M2 | Filters correct before ranking/limits; zero excluded records returned | CLI/UI/MCP tests pass; the prior `0af8643` [guest GUI journey](validation/desktop-gui.md) exercised project, source, Unassigned, and time filters; styled source panel passed local project/source checks; final styled packaged GUI remains untested | Partial; final GUI pending |
...
| M4 | Retention/exclusions/consent/pause behave as documented; experiments off by default | Final CLI suite covers consent epochs, read-time clipboard revocation, retention, and pause/resume; the prior `d1c3e3b` [stripped-guest run](validation/packaged-app-2026-09-23.md) proved pre-consent clipboard exclusion, later-copy capture, and pause/source choices across a graceful reboot. An immediate host Tart stop rolled back recent guest writes. The final ZIP did not repeat reboot. | Source and prior artifact passed; final reboot pending |
...
| U1 | Actual keyboard/menu → filter/preview/copy/open/assign/forget flow works; stale searches cannot overwrite state | Prior `0af8643` [installed guest journey](validation/desktop-gui.md) passed the full menu/filter/copy/open/assign/forget path; final styled source panel passed local filter/preview/copy/assign/pause/keyboard checks with synthetic data. Cross-app shortcut, rapid visual stale race, and final styled packaged GUI remain unproven | Partial; final GUI pending |
...
| P1 | Standalone app works without development tools; install/upgrade/uninstall preserve chosen state | Exact `b603442` ZIP [build and stripped-guest checks](../packages/cli/packaging/README.md) passed offline recall, targeted error/recovery paths; prior `d1c3e3b` [guest run](validation/packaged-app-2026-09-23.md) passed daemon, consent, graceful reboot, uninstall/key preservation, and locked-Keychain fail-fast. Legacy 0.1 wrapped-key approval and final packaged GUI remain unproven | Partial; upgrade pending |
...
| E1 | Integrated cross-surface journey and 2-hour candidate soak pass; resource behavior reported | CLI/MCP/delete/restart journey and the prior installed desktop journey passed; exact `b603442` 7,200-second source soak remains in progress. Final styled packaged desktop journey is still open. | Pending soak and final desktop |
