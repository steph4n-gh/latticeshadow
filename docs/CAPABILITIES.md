...
| Background capture | An opt-in daemon reads future clipboard text and Zsh history entries; `shadow status` and `shadow disable` expose and stop daemon state. | The source flags for both are on by default once the daemon starts; wizard completion is not enforced. Password-manager concealment and secret filters are incomplete boundaries. Fresh-install and upgrade behavior needs more live testing. |
