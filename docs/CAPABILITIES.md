...
| Menu bar and native companion | A recall panel with scope filters, preview, copy/open, project assignment, and confirmed forget is implemented. | A [logged-in guest journey](validation/desktop-gui.md) passed on a prior artifact, and the styled panel passed a local AppKit check with synthetic data. The styled packaged GUI and cross-app global shortcut still need checks. |
| Releases | Tags publish source archives. The rebuilt unsigned Apple Silicon app passed [final packaged recall and fresh-guest recovery checks](validation/mini-b603-packaged-2026-09-23.md). [Daemon, consent, graceful reboot, and locked-key checks](validation/packaged-app-2026-09-23.md) passed on an earlier candidate. | Legacy upgrade Keychain approval, styled packaged GUI, stock Gatekeeper, signing, and notarization remain before a public binary release. |
