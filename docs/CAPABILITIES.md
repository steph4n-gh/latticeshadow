...
| Menu bar and native companion | A recall panel with scope filters, preview, copy/open, project assignment, and confirmed forget is implemented. | The [current ZIP passed a logged-in menu/Recall/filter/preview/Copy smoke](validation/desktop-gui.md); a preceding package passed the fuller Open/Assign/Forget journey. The cross-app global shortcut remains unproven. |
| Releases | Tags publish source archives. The current unsigned Apple Silicon app passed [packaged recall, query timing, fresh-guest recovery, and explicit daemon/reboot/removal checks](validation/packaged-app-20669ff-2026-09-23.md) plus a [logged-in GUI smoke](validation/desktop-gui.md). | Populated 0.1 upgrade remains unverified after an ad-hoc-signature Keychain access failure. Stock Gatekeeper, signing, and notarization remain before a public binary release. |
