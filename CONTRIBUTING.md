...
checks: `make test-db`, `make test-cli` on macOS, and `make docs`. CI also runs the
CLI model check and native companion build on macOS. Hardware tests are opt-in.
