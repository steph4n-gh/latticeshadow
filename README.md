...
| Desktop and peer features | The current ZIP passed a [logged-in menu, Recall, filter, preview, Copy, and consent-control smoke](docs/validation/desktop-gui.md). A preceding package passed the fuller file Open, assignment, and confirmed Forget journey. Cross-app shortcut behavior remains unproven. Cross-device sync, autonomous repair, and several retrieval modes remain experimental. |
| Distribution | Source releases are public. The current unsigned Apple Silicon app passed [packaged recall, query timing, fresh-guest recovery, and explicit daemon/reboot/removal checks](docs/validation/packaged-app-20669ff-2026-09-23.md) plus a [logged-in GUI smoke](docs/validation/desktop-gui.md). Populated 0.1 upgrade remains unverified after an ad-hoc-signature Keychain access failure; stock Gatekeeper, signing, and notarization evidence remain open. |
