...
| Background capture | macOS clipboard and terminal capture require explicit source choices and `shadow enable`. Pause persists across an ordinary guest reboot; literal/source exclusions and optional age retention are implemented. Logged-in reboot behavior remains to be checked. |
...
| Desktop and peer features | The menu-bar Recall panel passed a [logged-in synthetic guest journey and a styled local AppKit check](docs/validation/desktop-gui.md). Cross-app shortcut behavior and the styled packaged GUI remain to be checked. Cross-device sync, autonomous repair, and several retrieval modes remain experimental. |
| Distribution | Source releases are public. The rebuilt unsigned Apple Silicon app passed [final packaged recall and fresh-guest recovery checks](docs/validation/mini-b603-packaged-2026-09-23.md). [Daemon, consent, graceful reboot, and key-preservation checks](docs/validation/packaged-app-2026-09-23.md) passed on an earlier candidate. Legacy upgrade approval, stock Gatekeeper, signing, and notarization evidence remain open. |
