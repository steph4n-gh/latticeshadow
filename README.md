...
| Desktop and peer features | The tested ZIP passed a [logged-in menu, Recall, filter, preview, Copy, file Open, assignment, confirmed Forget, and consent-control journey](docs/validation/desktop-gui.md). Return on a focused result row did not copy in that guest; a source fix and focused test pass, with packaged retest pending. Cross-app shortcut behavior remains unproven. Cross-device sync, autonomous repair, and several retrieval modes remain experimental. |
| Distribution | Source releases are public. The current unsigned Apple Silicon app passed [packaged recall, query timing, fresh-guest recovery, and explicit daemon/reboot/removal checks](docs/validation/packaged-app-20669ff-2026-09-23.md) plus a [logged-in GUI journey](docs/validation/desktop-gui.md). Populated 0.1 upgrade remains unverified after an ad-hoc-signature Keychain access failure; stock Gatekeeper, signing, and notarization evidence remain open. |
